...
def load_llm_config(profile="ob_xianzi"):
    ...
    # 3. 若 .env 配置了 SENSE_API_KEY + SENSE_BASE_URL，优先使用 SenseNova
    #    （base_url/model/api_key 全部替换；不再依赖 opencode.ai 端点）
    if 'SENSE_API_KEY' in env_vars and 'SENSE_BASE_URL' in env_vars:
        llm_config['base_url'] = env_vars['SENSE_BASE_URL'].rstrip('/')
        llm_config['api_key'] = env_vars['SENSE_API_KEY']
        llm_config['model'] = 'sensenova-6.7-flash-lite'

    # 4. 从 .env 读取 API key（如果前几步未提供）
    ...
    # 5. OPENAI_BASE_URL 覆盖（如果 .env 中有且未被注释）
    if 'OPENAI_BASE_URL' in env_vars:
        llm_config['base_url'] = env_vars['OPENAI_BASE_URL'].rstrip('/')

    # 6. 如果用 xunfei/astron 端点，使用正确的模型名
    if 'xf-yun' in llm_config['base_url'] or 'maas-coding' in llm_config['base_url']:
        llm_config['model'] = 'astron-code-latest'

    # 7. 若 .env 配置了 AGNES（且未配置 SENSE），则 link2obsidian 的 LLM 摘要使用 agnes-2.0-flash
    #    （仅影响本技能的摘要功能；其他抓取/图片/后处理逻辑保持不变）
    #    注意：SENSE 优先于 AGNES——SENSE 是国内直连端点（token.sensenova.cn），更稳定；
    #    AGNES 端点（apihub.agnes-ai.com）曾出现 TCP 不可达，仅作 SENSE 缺失时的 fallback。
    if 'AGNES_BASE_URL' in env_vars and 'AGNES_API_KEY' in env_vars and 'SENSE_API_KEY' not in env_vars:
        llm_config['base_url'] = env_vars['AGNES_BASE_URL'].rstrip('/')
        llm_config['api_key'] = env_vars['AGNES_API_KEY']
        llm_config['model'] = 'agnes-2.0-flash'
